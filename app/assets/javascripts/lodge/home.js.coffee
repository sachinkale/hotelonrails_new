	...
	printCurrent = (c) ->
		...
		if($('#invoice-' + c + ' .checked_out') != null)
			$.each $('#invoice-' + c + ' .checked_out'), (key,val) ->
				...
				w.document.write("<br/>Checked out at: ")
				w.document.write($(val).find(' .checkout-date').html())
		...
		if($('#include_checkout_date').prop('checked'))
			w.document.write("<br/>Checkout Date: ")
			w.document.write(new Date())
		w.document.write("<table rules=all border=1 cellpadding=5>")
		...
		w.document.write("</h4>")
		if($('#include_payments').prop('checked'))
			w.document.write("<h4> Payments </h4>")
			w.document.write("<table rules=all border=1 cellpadding=5>")
			w.document.write($('#invoice-' + c + ' .well.payments table').html())
			w.document.write("</table>")
		if($('#include_summary').prop('checked'))
			w.document.write("<h4> Total </h4>")
			w.document.write("<table rules=all border=1 cellpadding=5>")
			w.document.write($('#invoice-' + c + ' .well.total_charges table').html())
			w.document.write("</table>")
	
		w.document.write($('.tax-declaration').html())
		w.document.write("</body></html>")
		w.print()
		if($('#print_other_charges').prop('checked'))
